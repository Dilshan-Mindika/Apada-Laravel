                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div v-for="person in people.data" :key="person.id" class="border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div class="p-4">
                                    ...
                                    <p class="text-sm text-gray-500 mt-2 line-clamp-2">{{ person.description }}</p>
                                    
                                    <div class="mt-4 pt-4 border-t border-gray-100">
                                        <p class="text-xs text-gray-500 uppercase font-semibold">{{ trans.contact }}</p>
                                        <p class="text-sm font-medium">{{ person.contact_name }}</p>
                                        <div class="flex flex-col">
                                            <a :href="'tel:' + person.contact_phone" class="text-sm text-blue-600 hover:underline">{{ person.contact_phone }}</a>
                                            <a v-if="person.contact_phone_2" :href="'tel:' + person.contact_phone_2" class="text-sm text-blue-600 hover:underline">{{ person.contact_phone_2 }}</a>
                                        </div>
                                    </div>

                                    <div class="mt-4" v-if="person.status === 'missing'">
                                        <button @click="markFound(person.id)" class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-green-700 bg-green-100 hover:bg-green-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                            {{ trans.mark_found }}
                                        </button>
                                    </div>
                                </div>
